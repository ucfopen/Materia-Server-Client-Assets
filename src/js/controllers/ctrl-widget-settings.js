...
app.controller('WidgetSettingsController', function($scope, $filter, $window, selectedWidgetSrv, widgetSrv, Alert) {
	...
	$scope.toggleNormalAccess = function() {
		if ($scope.guestAccess = true) { $scope.guestAccess = false; }
		if ($scope.embeddedOnly = true) { return $scope.embeddedOnly = false; }
	};
	...
	$scope.toggleGuestAccess = function() {
		...
		if ($scope.guestAccess) { $scope.embeddedOnly = false; }
	...
	$scope.toggleEmbeddedOnly = function() {
		...
		if ($scope.embeddedOnly) { return $scope.guestAccess = false; }
	};
	...
	$scope.parseSubmittedInfo = function() {
		...
		for (let range of Array.from(ranges)) {
			...
			if (anytime === true) {
				...
				if ((monthDayYear != null) && (monthDayYear.length === 3)) {
					if ((monthDayYear[0].length > 2) || (monthDayYear[1].length > 2) || (monthDayYear[2].length > 4)) {
						errors.date++;
						errors.invalid++;
		...
		if ((errors.date > 0) || (errors.time > 0)) {
			$scope.error = 'The';
			if (errors.date > 0) { $scope.error += " date"; }
			if (errors.date > 1) { $scope.error += "s"; }
			if ((errors.time > 0) && (errors.date > 0)) { $scope.error += " and"; }
			if (errors.time > 0) { $scope.error += " time"; }
			if (errors.time > 1) { $scope.error += "s"; }
			if (((errors.date > 0) && (errors.time > 0)) || (errors.date > 1) || (errors.time > 1)) {
				$scope.error += " are ";
			} else {
				$scope.error += " is ";
			}
			if (errors.invalid > 0) { $scope.error += "invalid"; }
			if ((errors.invalid > 0) && (errors.missing > 0)) { $scope.error += "/"; }
			if (errors.missing > 0) { $scope.error += "missing"; }
			$scope.error += ".";
		}
	...
	$scope.changeAvailability = function() {
		...
		const attempts = $scope.attemptsSliderValue < $scope.UNLIMITED_SLIDER_VALUE ? $scope.attemptsSliderValue : -1;
		...
		return widgetSrv.saveWidget({
			...
			, function(widget) {
				...
				return selectedWidgetSrv.updateAvailability(attempts, $scope.times[0], $scope.times[1], $scope.guestAccess, $scope.embeddedOnly);
		});
	};
