...
app.controller('playerCtrl', function($scope, $sce, $timeout, widgetSrv, userServ, PLAYER, Alert) {
	...
	const onWidgetReady = function() {
		...
		switch (false) {
			case !(qset == null): return embedDoneDfD.reject('Unable to load widget data.');
			case !(widget == null): return embedDoneDfD.reject('Unable to load widget.');
			default: return embedDoneDfD.resolve();
		}
	};
	...
	const sendStorage = function(log) {
		if (!$scope.isPreview) { return pendingLogs.storage.push(log); }
	};
	...
	const end = function(showScoreScreenAfter) {
		...
		switch (endState) {
			case 'sent':
				if (showScoreScreenAfter) { return showScoreScreen(); }
				break;
			case 'pending':
				if (showScoreScreenAfter) { return scoreScreenPending = true; }
				break;
			default:
				endState = 'pending';
				// kill the heartbeat
				...
				return sendAllPendingLogs(function() {
					...
					if (showScoreScreenAfter || scoreScreenPending) { return showScoreScreen(); }
				});
		}
	};
		...
		setInterval(() =>
			Materia.Coms.Json.send('session_play_verify', [play_id], function(result) {
				if ((result !== true) && (instance.guest_access === false)) {
					return _alert('Your play session is no longer valid! This may be due to logging out, your session expiring, or trying to access another Materia account simultaneously. You\'ll need to reload the page to start over.', 'Invalid session', true);
				}
			})
	...
	const sendWidgetInit = function() {
		...
		sendToWidget('initWidget', widgetType === '.swf' ? [qset, convertedInstance] : [qset, convertedInstance, BASE_URL, MEDIA_URL]);
		if (!$scope.isPreview) {
	...
	var sendToWidget = function(type, args) {
		switch (widgetType) {
			...
			case '.html':
				return widget.contentWindow.postMessage(JSON.stringify({type, data:args}), STATIC_CROSSDOMAIN);
		}
	};
	...
	const embed = function() {
		...
		if (instance.widget.width > 0) { $('.preview-bar').width(instance.widget.width); }
	...
	var embedFlash = function(enginePath, version, dfd) {
		...
		$scope.type = "flash";
		$scope.$apply();
		return swfobject.embedSWF(enginePath, PLAYER.EMBED_TARGET, width, height, String(version), express, flashvars, params, attributes);
	};
	...
	var embedHTML = function(enginePath, dfd) {
		...
		if (instance.widget.width > 0) { $(`#${PLAYER.EMBED_TARGET}`).width(instance.widget.width); }
		if (instance.widget.height > 0) { $(`#${PLAYER.EMBED_TARGET}`).height(instance.widget.height); }
		...
		const _onPostMessage = function(e) {
			if (e.origin === expectedOrigin) {
				...
				switch (msg.type) {
					case 'start':           return onWidgetReady();
					case 'addLog':          return addLog(msg.data);
					case 'end':             return end(msg.data);
					case 'sendStorage':     return sendStorage(msg.data);
					case 'sendPendingLogs': return sendAllPendingLogs();
					case 'alert':           return _alert(msg.data, 'Warning!', false);
					case 'setHeight':       return setHeight(msg.data[0]);
					case 'initialize':
					default:                   throw new Error(`Unknown PostMessage received from player core: ${msg.type}`);
				}
			} else {
	...
	const getWidgetInstance = function() {
		...
		if ($scope.type === "noflash") { dfd.reject('Flash Player required.'); }

		widgetSrv.getWidget($scope.inst_id, function(widgetInstances) {
			if (widgetInstances.length < 1) { dfd.reject('Unable to get widget info.'); }
			...
			for (let feature of Array.from(instance.widget.meta_data.features)) {
				if (feature.toLowerCase() === "fullscreen") { $scope.allowFullScreen = true; }
			}
			...
			if ((type === 'swf') && (swfobject.hasFlashPlayerVersion(String(version)) === false)) {
				...
				if (instance.widget.width > 0) { $('.center').width(instance.widget.width); }
				if (instance.widget.height > 0) { $('.center').height(instance.widget.height); }
				dfd.resolve();
			}
	...
	const startPlaySession = function() {
		...
		switch (false) {
			case $scope.type !== "noflash": dfd.reject('Flash Player Required.'); break;
			case !$scope.isPreview: dfd.resolve(); break;
			default:
	...
	var pushPendingLogs = function() {
		...
		return (Materia.Coms.Json.send('play_logs_save', pendingQueue[0].request, function(result) {
			...
			if ($scope.alert.fatal) { $scope.alert.fatal = false; }
	...
	var sendPendingPlayLogs = function() {
		...
		if (pendingLogs.play.length > 0) {
			...
			if ($scope.isPreview) { args.push($scope.inst_id); }
	...
	var setHeight = function(h) {
		...
		if (h > min_h) { return $('.center').height(h); } else { return $('.center').height(min_h); }
	};
	...
	var showScoreScreen = function() {
		...
		if (!$scope.alert.fatal) { return window.location = scoreScreenURL; }
	};
	...
	({
		...
		link($scope, $element, $attrs) {
			return $scope.$watch("allowFullScreen", function(newVal, oldVal) {
				if (newVal === true) { return $attrs.$set("allowfullscreen", ""); }
			});
		}
	})
);
