...
app.service('dateTimeServ', function() {
	...
	const fixTime = function(time, servTime) {
		const timeToFix = new Date(time).getTime();
		const serverDateFromPage = servTime;

		// calculate client/server time difference
		const now = new Date();
		const clientUTCDate = new Date(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(),  now.getUTCHours(), now.getUTCMinutes(), now.getUTCSeconds());
		...
		if (fullHour > 11) { fixedDateStr += 'pm';
